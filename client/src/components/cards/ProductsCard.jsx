...
const ProductsCard = ({ product }) => {
  ...
  const addFavourite = async (e) => {
    ...
    if (!token) {
      dispatch(openSnackbar({ message: "Please login to add to favorites", severity: "error" }));
      return;
    }
  ...
  const addCart = async (e) => {
    ...
    const token = localStorage.getItem("foodeli-app-token");
    if (!token) {
      dispatch(openSnackbar({ message: "Please login to add to cart", severity: "error" }));
      return;
    }
  ...
  return (
    <Card>
      <Top>
        ...
        <Menu>
          <MenuItem
            onClick={(e) => (favorite ? removeFavourite(e) : addFavourite(e))}
          >
            {favoriteLoading ? (
              <>
              ...
              <>
                {favorite ? (
                  <FavoriteRounded sx={{ fontSize: "24px", color: "red" }} />
                ) : (
